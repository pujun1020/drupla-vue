...
onMounted(async () => {
    ...
    let debounceupdateScroll = function () {
        ...
        <div class="row">
            ...
            <div class="content col-xl-9 col-lg-9 col-md-12 col-sm-12" ref="sectionparent">
                <section v-for="(item, index) in articles" :key="item.id" :id="item.attributes.field_biao_ti_id">
                    ...
                    <div class="contents-summary" v-show="item.attributes.title">{{ item.attributes.body.summary }}
                    </div>
                    <div class="image-wapper"
                        ...
                        <img :src="`http://8.134.37.43:5175${articleimage.filter((items: any) => items.id === item.relationships.field_image.data.id)[0].attributes.uri.url}`"
                            alt="">
                    </div>
